...
- **Domain:** `airfi`
- **Title:** Airfi
- **Repository:** janmilinds/ha-airfi

**Key directories:**

- `custom_components/airfi/` - Main integration code
...
- Integration logs: `custom_components.airfi: debug` in `config/configuration.yaml`
...
- Use the domain `airfi` for all DOMAIN references
- Prefix all integration-specific classes with `Airfi`
- Use "Airfi" as the display title
...
- Inherit from platform base + `AirfiEntity`
...
- `tests/` mirrors `custom_components/airfi/` structure
