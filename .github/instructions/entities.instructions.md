...
- **`entity/`** - Base entity classes (inherit `AirfiEntity` from `entity/base.py`)
...
**MUST inherit from:** `(PlatformEntity, AirfiEntity)` - order matters for MRO
...
**Imports pattern:** `from homeassistant.components.PLATFORM import PlatformEntity, PlatformEntityDescription` + `from ..entity import AirfiEntity`
