...
This document describes the technical architecture of the Airfi custom component for Home Assistant.
...
- `AirfiConfigFlowHandler` (main flow)
- `AirfiOptionsFlow` (options)
...
**Key class:** `AirfiEntity` (in `entity/base.py`)
...
2. `AirfiEntity` for common functionality
...
1. Create directory: `custom_components/airfi/<platform>/`
2. Implement `__init__.py` with `async_setup_entry()`
3. Create entity classes inheriting from platform base + `AirfiEntity`
